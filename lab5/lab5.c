...
void * driving_thread(void * args){
    ...
    sprintf(target_str, "car %d: %d,%d->%d,%d->%d,%d==>%d,%d",
        car_id, uber.data[0], uber.data[1], request.data[0], request.data[1], request.data[2], request.data[3], uber.data[0], uber.data[1]);
    int i ;
    for (i = 1; i < 5; i++) {
        sleep(4) ;
